...
/** Parse NCU CSV rows by metric names, independent of column order and optional metrics. */
export function parseNcuCsv(text: string, kernelName: string, rawReportPath?: string): ProfilerObservation {
  const records = parseCsv(text)
  const headerIndex = records.findIndex(row => row.some(cell => cell.trim() === 'Metric Name'))
  if (headerIndex < 0) throw new Error('NCU CSV does not contain a Metric Name header')
  ...
  const rows: MetricRow[] = records.slice(headerIndex + 1).flatMap(row => {
